    ...
    def write_data(self, mac, sensor_type, payload):
        ...
        if sensor_type == "acceleromete":
            ...
            if isinstance(value, dict):
                ...
                logger.warning(f"Invalid accelerometer data from {mac} | Payload: {json.dumps(payload, indent=2)}")
                return
        else:
...
def on_message(client, userdata, msg):
    try:
        ...
        logger.error(f"Data validation error: {str(e)}| Payload: {json.dumps(payload, indent=2)}")
    except Exception as e:
        logger.error(f"Unexpected error: {str(e)}| Payload: {json.dumps(payload, indent=2)}")
